...
type CascadeInfo struct {
	...
	p0         vector.Vec2 // Position of the first probe in this cascade.
...
func NewCascadeCalculator(width, height int) *CascadeCalculator {
	...
	for i := 0; i < cc.NCascades+1; i++ {
		...
		if i == 0 {
			...
			cc.CascadeInfo[i].p0 = vector.Vec2{
...
func (ci *CascadeInfo) GetProbe(i int, j int, index int) CascadeProbe {
	...
	ray := vector.Ray2D{P: vector.Vec2{
...
func (ci *CascadeInfo) GetProbeCenter(i int, j int) vector.Vec2 {
	p := vector.Vec2{
