...
func NewSceneShadows(time float64) *sdf.Scene {
	...
	for i := 0; i < 10; i++ {
		...
		s.Objects = append(s.Objects, &signed_distance.Circle{Center: vector.Vec2{X: x, Y: y}, Radius: float64(i)/100. + 0.01, Material: primitives.NewAbsorbiveMaterial(1000, 0, 0, 0)})
	}
	return s
}
