...
func ToImage(grid [][]Cell, image *primitives.SampledImage) {
	b0_base := math.Sqrt(1. / (2. * math.Pi))
	for i := range grid {
		for j := range grid[i] {
			// simple average of the harmonics
			c := grid[i][j]
			image.SetColor(j, i, primitives.Color{
				R: c.r.b0 * b0_base,
				G: c.g.b0 * b0_base,
				B: c.b.b0 * b0_base,
			})
...
func ToGrid(scene scene.Scene, image *primitives.SampledImage) [][]Cell {
	var grid [][]Cell
	grid = make([][]Cell, image.Height)
	for i := range grid {
		grid[i] = make([]Cell, image.Width)
		for j := range grid[i] {
			grid[i][j] = Cell{m: scene.GetMaterial(primitives.Vec2{
...
func PropagateFrom(n primitives.Vec2, sa float64, src *Cell, dest *Cell) {
...
func Propagate(grid [][]Cell, grid2 [][]Cell, width, height int) {
	...
	dn1 := primitives.Vec2{X: 0.8944271909999159, Y: 0.4472135954999579}  // normal to the center of the top side of our pixel
	dn0 := primitives.Vec2{X: 0.8944271909999159, Y: -0.4472135954999579} // normal to the center of the bottom side of our pixel
	...
	for j := 1; j < height-1; j++ {
		for i := 1; i < width-1; i++ {
			...
			src = &grid[j][i-1]
			PropagateFrom(primitives.Vec2{X: 1, Y: 0}, sa1, src, dst)
			PropagateFrom(primitives.Vec2{X: dn0.X, Y: dn0.Y}, sa2, src, dst)
			PropagateFrom(primitives.Vec2{X: dn1.X, Y: dn1.Y}, sa2, src, dst)
			...
			src = &grid[j-1][i]
			PropagateFrom(primitives.Vec2{X: 0, Y: 1}, sa1, src, dst)
			PropagateFrom(primitives.Vec2{X: dn0.Y, Y: dn0.X}, sa2, src, dst)
			PropagateFrom(primitives.Vec2{X: dn1.Y, Y: dn1.X}, sa2, src, dst)
...
func LightPropagationVolume(scene scene.Scene, image *primitives.SampledImage) {
